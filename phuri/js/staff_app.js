...
function loadStaffGlobalData(staffId) {
    // --- (จำลองการยิง API) ---
    /*
    fetch(`https://your-backend-api.com/staff/${staffId}/header`)
        .then(response => response.json())
        .then(data => {
            if (document.getElementById('username')) {
                document.getElementById('username').textContent = `นพ. ${data.firstname}`; 
            }
            if (document.getElementById('userId')) {
                document.getElementById('userId').textContent = data.staffId;
            }
        })
        .catch(error => console.error("Error loading staff global data:", error));
    */
    ...
    if (document.getElementById('username')) {
        document.getElementById('username').textContent = `นพ. ${testData.firstname}`;
    }
    if (document.getElementById('userId')) {
        document.getElementById('userId').textContent = testData.staffId;
    }
    // --- (จบส่วนทดสอบ) ---
}
...
function loadStaffProfileData(staffId) {
    ...
    populateStaffProfileForm(testData);
    
    // ตั้งค่าเป็น "อ่านอย่างเดียว" ตอนเริ่มต้น (ยกเว้นเบอร์โทร)
 ...
 */
function saveStaffProfileData(staffId) {
    const dataToSave = {
        staffId: staffId,
    ...
    fetch(`https://your-backend-api.com/staff/${staffId}/profile`, {
        method: 'POST', 
...
function loadAllPatients(staffId) {
    ...
    /*
    fetch(`https://your-backend-api.com/staff/${staffId}/patients`)
        .then(response => response.json())
        .then(data => {
            allPatientsData = data.patients; // เก็บข้อมูลต้นฉบับไว้
            renderPatientList(allPatientsData); // แสดงผลครั้งแรก
        })
        ...
        card.innerHTML = `
            <div class="patient-info">
                ...
                <div class="patient-details">
                    ...
                    <span class="patient-age">อายุ: ${patient.age}</span>
                    <span class="patient-blood">กรุ๊ปเลือด: ${patient.blood}</span>
                </div>
            </div>
